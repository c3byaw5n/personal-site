...
<template>
  <TheContentContainer>
    <div class="mx-auto max-w-3xl pb-20">
      <h1 class="mb-8 text-center text-3xl tracking-widest text-fuchsia-950 md:text-4xl">
        Fragment
      </h1>

      <div class="mb-8 space-y-3 md:space-y-4">
        ...
        <div v-if="availableTags.length > 0" class="flex min-h-10 flex-col items-center">
          <Transition
            mode="out-in"
            ...
            <div v-else id="tags-panel" class="flex w-full flex-col items-center">
              <div class="flex flex-wrap justify-center gap-3">
                ...
                <button
                  ...
                  :key="tag"
                  class="rounded-full border px-4 py-1.5 text-xs tracking-wider uppercase transition-colors"
                  :class="
                    selectedTag === tag
                      ? 'border-fuchsia-900 bg-fuchsia-900 text-white'
                      : 'border-fuchsia-900/20 text-fuchsia-900/60 hover:bg-fuchsia-50'
              ...
              <button
                class="group mt-6 flex items-center gap-1 px-5 text-[12px] font-bold tracking-widest text-fuchsia-900/50 transition-colors hover:text-fuchsia-900"
      ...
      <div v-if="displayedPosts.length > 0" class="space-y-12">
      ...
      <div v-else class="py-20 text-center tracking-widest text-fuchsia-900">
        該当する記事が見つかりませんでした。
      </div>

      <div v-if="hasMore" class="mt-16 text-center">
        <button
          class="group inline-flex items-center gap-2 rounded-full border border-fuchsia-900/40 px-8 py-3 text-sm tracking-widest text-fuchsia-900/80 transition-all hover:border-fuchsia-900/60 hover:bg-fuchsia-50 hover:text-fuchsia-900"
          @click="loadMore"
        >
          もっと見る
